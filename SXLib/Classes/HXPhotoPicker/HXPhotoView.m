...
- (void)setupNewFrame {
    ...
    if (itemW > 0) {
        self.flowLayout.itemSize = CGSizeMake(itemW, itemW*self.proportion);
    }
    ...
    if (numOfLinesNew != self.numOfLinesOld) {
        CGFloat newHeight = numOfLinesNew * itemW*self.proportion + self.spacing * (numOfLinesNew - 1);
        if (newHeight < 0) {
            newHeight = 0;
        }
...
- (void)layoutSubviews {
    ...
    if (dataCount == 1) {
        ...
        if ((int)height != (int)itemW) {
            self.frame = CGRectMake(x, y, width, itemW*self.proportion);
        }
    }
    if (dataCount % self.lineCount == 0) {
        numOfLinesNew -= 1;
