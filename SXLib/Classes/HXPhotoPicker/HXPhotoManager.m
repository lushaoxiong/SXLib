...
- (NSString *)maximumOfJudgment:(HXPhotoModel *)model {
    ...
    if (model.type == HXPhotoModelMediaTypeVideo) {
        ...
        return [NSBundle hx_localizedStringForKey:@"您选取的图片类型暂不支持~"];
    }
    return nil;
}

- (void)maximumOfJudgment:(HXPhotoModel *)model callback:(void(^)(NSString *result))callback
{
    if (model.type != HXPhotoModelMediaTypePhoto) {
        callback( [self maximumOfJudgment:model] );
    }else{
        if([[model.fileURL.absoluteString lowercaseString] containsString:@"png"]||[[model.fileURL.absoluteString lowercaseString] containsString:@"jpg"]||[[model.fileURL.absoluteString lowercaseString] containsString:@"jpeg"]){
            ...
            [imageManager requestImageDataForAsset:model.asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                ...
                CGFloat lg = data.length/1024;
                CGFloat h =lg/1024.00;
                if(h>30.00){
                    callback([NSBundle hx_localizedStringForKey:@"图片太大了，换张试试~"]);
                }else{
                    callback( [self maximumOfJudgment:model] );
                }
            }];
            ...
            callback([NSBundle hx_localizedStringForKey:[NSString stringWithFormat:@"您选取的图片类型暂不支持~"]]);
        }
    }
}
