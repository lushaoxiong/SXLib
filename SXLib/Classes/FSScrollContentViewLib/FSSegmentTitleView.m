...
- (void)layoutSubviews
{
    ...
    totalBtnWidth += self.edgeInset.left;
    ...
    totalBtnWidth += self.edgeInset.right;
    
    
    CGFloat currentX = self.edgeInset.left;
    CGFloat itemBtnHeight = CGRectGetHeight(self.bounds);
    for (int idx = 0; idx < self.titlesArr.count; idx++) {
        UIButton *btn = self.itemBtnArr[idx];
        titleFont = btn.isSelected?_titleSelectFont:_titleFont;
        CGFloat itemBtnWidth = [FSSegmentTitleView getWidthWithString:self.titlesArr[idx] font:titleFont];
        btn.frame = CGRectMake(currentX, 0, itemBtnWidth, itemBtnHeight);
        currentX = currentX + itemBtnWidth + self.itemMargin;
    }
    currentX += self.edgeInset.right;
    ...
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (_indicatorType) {
            ...
            case FSIndicatorTypeCustom:
                ...
                break;
            case FSIndicatorTypeFix:
                self.indicatorView.center = CGPointMake(selectBtn.center.x, CGRectGetHeight(self.scrollView.bounds) - 1);
                self.indicatorView.bounds = CGRectMake(0, 0, _indicatorWidth, 2);
                break;
...
- (void)setSubTitle:(NSString *)subText atIndex:(NSInteger)index
{
    if (index >= self.itemBtnArr.count || index >= self.titlesArr.count) {
        return;
    }
    
    UIButton *btn =  self.itemBtnArr[index];
    NSString *title =  [self.titlesArr[index] stringByTrimmingCharactersInSet: [NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if (!subText || [subText isEqualToString:@""]) {
        NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:title];
        [str addAttribute:NSForegroundColorAttributeName value:_titleNormalColor range:NSMakeRange(0,title.length)];
        [str addAttribute:NSFontAttributeName value:_titleFont range:NSMakeRange(0, title.length)];
        
        [btn setAttributedTitle:str forState:UIControlStateNormal];
        
        NSMutableAttributedString *selectStr = [str mutableCopy];
        [selectStr removeAttribute:NSForegroundColorAttributeName range:NSMakeRange(0,title.length)];
        [selectStr addAttribute:NSForegroundColorAttributeName value:_titleSelectColor range:NSMakeRange(0,title.length)];
        [btn setAttributedTitle:selectStr forState:UIControlStateSelected];
    }else{
        NSString *numTitle = [NSString stringWithFormat:@"%@  %@",title,subText];
        NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:numTitle];
        [str addAttribute:NSForegroundColorAttributeName value:_titleNormalColor range:NSMakeRange(0,numTitle.length)];
//        [str addAttribute:NSForegroundColorAttributeName value:_titleNormalColor range:NSMakeRange(0,title.length)];
//        [str addAttribute:NSForegroundColorAttributeName value:UIColorFromRGB(0x9f9f9f) range:NSMakeRange(title.length,subText.length+2)];
        [str addAttribute:NSFontAttributeName value:_titleFont range:NSMakeRange(0, title.length)];
        [str addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:11] range:NSMakeRange(title.length,subText.length+2)];
        
        [btn setAttributedTitle:str forState:UIControlStateNormal];
        
        NSMutableAttributedString *selectStr = [str mutableCopy];
        [selectStr removeAttribute:NSForegroundColorAttributeName range:NSMakeRange(0,numTitle.length)];
        [selectStr addAttribute:NSForegroundColorAttributeName value:_titleSelectColor range:NSMakeRange(0,numTitle.length)];
        [btn setAttributedTitle:selectStr forState:UIControlStateSelected];
    }
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    [[UIColor colorWithRed:34/255.0 green:34/255.0 blue:34/255.0 alpha:1.0] set];
    
    CGContextMoveToPoint(ctx, 0 , rect.size.height);
    CGContextAddLineToPoint(ctx, self.bounds.size.width , rect.size.height);
