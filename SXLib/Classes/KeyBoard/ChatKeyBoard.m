...
+ (instancetype)keyBoardWithNavgationBarTranslucent:(BOOL)translucent
{
    ...
    //lsx
    CGFloat h =  [SXTool isHairHead]?20:0;
    if (translucent) {
        frame = CGRectMake(0, kScreenHeight - kChatToolBarHeight-h, kScreenWidth, kChatKeyBoardHeight);
    }else {
        frame = CGRectMake(0, kScreenHeight - kChatToolBarHeight -h- 64, kScreenWidth, kChatKeyBoardHeight);
    }
...
- (void)chatToolBarTextViewDidEndEditing:(UITextView *)textView;
{
    if ([self.delegate respondsToSelector:@selector(chatKeyBoardTextViewDidEndEditing:)]) {
        [self.delegate chatKeyBoardTextViewDidEndEditing:textView];
    }
}
...
- (void)keyboardDown
{
    if (self.keyBoardStyle == KeyBoardStyleChat)
    {
        ...
        {
            if(self.superview && ([self getSuperViewH] - CGRectGetMinY(self.frame)) > self.chatToolBar.frame.size.height)
            {
                [UIView animateWithDuration:0.25 animations:^{
...
- (CGFloat)getSuperViewH
{
    ...
    //lsx
     CGFloat h =  [SXTool isHairHead]?20:0;
    return self.superview.frame.size.height-h;
}
...
- (FacePanel *)facePanel
{
    if (!_facePanel) {
        ...
        //lsx
        _facePanel.backgroundColor=[UIColor clearColor];
    }
    return _facePanel;
}
...
- (MorePanel *)morePanel
{
    if (!_morePanel) {
        ...
        _morePanel.delegate = self;
         //lsx
        _morePanel.backgroundColor=[UIColor clearColor];
    }
    return _morePanel;
}
